...
def night_move(player,unit):
    global game_state
    close_home = get_closest_empty_city_tile(player,unit)
...
def build_city_action(game_state, player, unit):
    ...
    if unit.pos == build_location.pos:
        ...
        logging.info(f"build: Moving to build location {[move_dir]}")
...
def agent(observation, configuration):
    ...
    logging.info(f"here = {observation['step']}")
    ...
    for city in cities:
        for c_tile in city.citytiles:
            city_tiles.append([c_tile.pos.x, c_tile.pos.y ])
    ...
    for unit in player.units:
        ...
        if unit.is_worker() and unit.can_act():
            ...
            if build_city_flag and unit.cargo.wood >= 100:
    ...
    for ct in city.citytiles:
        if ct.can_act() and build_worker_flag:
    ...
    if night_flag:
        ...
        actions.append(annotate.text(0,0, "day",16))

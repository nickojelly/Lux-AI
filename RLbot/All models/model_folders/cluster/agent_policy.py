    ...
    def get_reward(self, game, is_game_finished, is_new_turn, is_game_error):
        ...
        for city in game.cities.values():
            if city.team == self.team:
                city_count += 1
                city_list[city] = (city.fuel,city.get_light_upkeep())
            else:
        ...
        # Gives reward for refueling a city up to 1.5x light upkeep for 10 nights
        for city,(fuel,need) in city_list.items():
            if city in self.city_dict_last.keys():
                if (fuel < 1.5*10*need)and(fuel>self.city_dict_last[city][1]):
                    rewards["rew/r_city_fuel"] = (fuel-self.city_dict_last[city][1])/10000
            
        self.city_dict_last = city_list
        # Give a reward for city creation/death. 0.1 reward per city.
        city_tile_change = (city_tile_count - self.city_tiles_last)
        cities_change = (city_count - self.city_count_last)
        if city_tile_change>0:
            if cities_change<city_tile_change:
                rewards["rew/r_city_tiles"] = city_tile_change * 0.2
            else:
                rewards["rew/r_city_tiles"] = city_tile_change * 0.1
        elif city_tile_change<0:
            rewards["rew/r_city_tiles"] = city_tile_change * 0.3
        self.city_tiles_last = city_tile_count
        self.city_count_last = city_count
        # city_tile_change = (city_tile_count - self.city_tiles_last)
        # rewards["rew/r_city_tiles"] = city_tile_change * 0.3
        # self.city_tiles_last = city_tile_count
        # Reward for reseraching
        research_points = game.state["teamStates"][self.team]["researchPoints"]
        if research_points<200:
            rewards["rew/research"] = (research_points-self.research_last)*0.01
        else:
            rewards["rew/research"] = (self.research_last-research_points)*0.01
        self.research_last = research_points
        # Reward collecting fuel
        fuel_collected = game.stats["teamStats"][self.team]["fuelGenerated"]
        rewards["rew/r_fuel_collected"] = ( (fuel_collected - self.fuel_collected_last) / 20000 )
        rewards["rew/coal"] = (game.stats["teamStats"][self.team]["resourcesCollected"]["coal"] - self.coal_last) / 20000
        self.coal_last = game.stats["teamStats"][self.team]["resourcesCollected"]["coal"]
        ...
        if is_game_finished:
            ...
            rewards["rew/r_city_tiles_end"] = city_tile_count*4
            if city_tile_count==0:
                rewards["rew/r_city_tiles_end"] = -100

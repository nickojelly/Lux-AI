...
def build_city_action(game_state, player, unit):
    ...
    if status.build_location:
        ...
        if unit.pos.distance_to(status.build_location.pos)>3:
...
def get_builder_workers(player):
    ...
    if build_cluster:
        ...
        if closest_unit:
            ...
            log_function(f"explorer b {build_location=}")
...
def agent(observation, configuration):
    ...
    #Generates a worker_status class for a worker
    workers = []
    ...
    #         home_city = get_closest_city_tile(player,worker)
    ...
    for unit in player.units:
        if status.night_flag:
            ...
            if build_city_flag and (unit.cargo.wood == 100) and unit.id == builder_unit.id:

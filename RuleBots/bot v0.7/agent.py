...
class WorkerStatus(Unit):
    ...
    def __init__(self, worker_obj) -> None:
        ...
        self.home_city = game_state.map.get_cell_by_pos(self.worker.pos).citytile
...
class CityStatus(City):
    ...
    def get_priority(self):
        if self.workers:
            priority = 1/len(self.workers)+int(self.obj.cityid[2])
        else:
            priority = 1
        return priority

    def pop_worker(self, explorer):
        if explorer.id in self.workers:
            self.workers.remove(explorer.id)
            log_function(f"{explorer.id} was removed from {self.obj.cityid}")
...
class CityWrapper(City):
    ...
    def remove_explorers(self, explorer):
        for city in self.city_dict.values():
            city.pop_worker(explorer)
...
def get_better_path(unit, dest, full_path = False):
    global game_state
    debug = False
    (dx, dy) = (dest.pos.x-unit.pos.x, dest.pos.y-unit.pos.y)
    better_path = None
    ...
    for c in cells:
        ...
        if not c.citytile and not c.has_resource():
            ...
            if not worker_dict[unit.id].cluster_builder:
                c_val = c_val - len(get_better_path(unit,c,True))
            for n in neighbours:
                if n.citytile:
                    c_val += 7
                if n.has_resource():
    ...
    if build_cluster:
        for unit in player.units:
            if not worker_dict[unit.id].build_new_cluster:
                try:
                    u_home = worker_dict[unit.id].home_city.cityid
                    log_function(f"{u_home=}")
                    log_function(f"{[x for x in city_wrapper.city_dict[u_home].workers]=}")
                    if len(city_wrapper.city_dict[u_home].workers)<=2:
                        log_function(f"Not Taking last worker away \n {u_home,city_wrapper.city_dict[u_home].workers=}")
                        continue
                except Exception as e:
                    log_function(f"{e=}")
        ...
        if closest_unit:
            ...
            city_wrapper.remove_explorers(unit)
    ...
    for worker in player.units:
        if worker.is_worker:
            ...
            if worker.id not in worker_dict.keys():
                ...
                home_city_id = new_worker.home_city.cityid
